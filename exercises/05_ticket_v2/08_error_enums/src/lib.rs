...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    ...
    if description.is_empty() {
        return Ticket { title, description: "Description not provided".into(), status }
    }
...
impl Ticket {
    pub fn new(
        ...
        if title.len() > 50 {
            return Err(TicketNewError::TitleError("Titls cannot be longer than 50 bytes".to_string()));
        }
        if description.is_empty() {
